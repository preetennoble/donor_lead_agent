...
#     # Sab sources ka text combine karo, priority-tagged
#     combined_text = ""
#     for s in sources:
...
#     # Apna existing Groq/Claude call pattern yahan use karo
#     raw_response = call_llm(prompt)   # jo bhi aapka function hai (Groq wala)
#     data = json.loads(raw_response)
#     data["source_url"] = "; ".join([s["url"] for s in sources[:5]])

#     return CompanyResearch(**data)  
#     api_key = (os.getenv("qroq_api") or os.getenv("GROQ_API_KEY") or "").strip().strip('"').strip("'")
#     if not api_key:
#         raise ValueError("Groq API Key not found in environment (qroq_api or GROQ_API_KEY)")
...
#     print(f"[Groq] Querying Groq for {company_name}...")
...
#     # Add source_url and ensure all fields have defaults
